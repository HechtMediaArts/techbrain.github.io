...
function graphy_logo() {
	...
	$logo_tag = ( is_front_page() && get_theme_mod( 'graphy_replace_blogname' ) ) ? 'div' : 'div';
...
?>
	...
	<?php if( is_single() ) : ?>
        Zuletzt bearbeitet am: <?php the_last_modified_timestamp(); ?>
    <?php else : ?>    
		<?php esc_html_e( 'Veröffentlicht', 'graphy' ) ?>
		<span class="posted-on"><?php esc_html_e( 'am', 'graphy' ); ?>
		...
		<?php endif; ?>
		<span class="byline"><?php esc_html_e( 'von', 'graphy' ); ?>
		...
		<?php if ( ! post_password_required() && comments_open() ) : ?>
			...
			<span class="comments-link">
				<?php comments_popup_link( esc_html__( '0 Kommentare', 'graphy' ), esc_html__( '1 Kommentar', 'graphy' ), esc_html__( '% Kommentare', 'graphy' ) ); ?>
			</span>
		<?php endif; ?>
	</div><!-- .entry-meta -->
	<?php
